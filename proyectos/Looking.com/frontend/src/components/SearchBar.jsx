...
export function StaysSearchBar({ values, onChange, onSearch }) {
  ...
  const provinces = COSTA_RICA_LOCATIONS.map((p) => p.name);
  const selectedProvince = COSTA_RICA_LOCATIONS.find(
    (p) => p.name === province,
  );
  const cantons = selectedProvince
    ? selectedProvince.cantons.map((c) => c.name)
    : [];
  const selectedCanton = selectedProvince?.cantons.find(
    (c) => c.name === canton,
  );
  const districts = selectedCanton
    ? selectedCanton.districts.map((d) => d.name)
    : [];
